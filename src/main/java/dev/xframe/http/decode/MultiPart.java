...
import dev.xframe.http.decode.HttpBody.SeekAheadNoBackArrayException;
import dev.xframe.http.decode.HttpBody.SeekAheadOptimize;
import dev.xframe.http.decode.HttpBody.TransferEncodingMechanism;
...
public class MultiPart {
    ...
    public MultiPart(HttpRequest request, ByteBuf byteBuf) {
        this.undecodedChunk = byteBuf;
        this.setMultipart(request.headers().get(HttpHeaders.Names.CONTENT_TYPE));
        this.parseBody();
    }
    ...
    private InterfaceHttpData decodeMultipart(MultiPartStatus state) {
        ...
        case FIELD: {
            ...
            Attribute nameAttribute = currentFieldAttributes.get(HttpBody.NAME);
            if (currentAttribute == null) {
                try {
                    currentAttribute = new MemoryAttribute(cleanString(nameAttribute.getValue()));
    ...
    private InterfaceHttpData findMultipartDisposition() {
        ...
        if (currentStatus == MultiPartStatus.DISPOSITION) {
            currentFieldAttributes = new TreeMap<CharSequence, Attribute>();
        }
        ...
        while (!skipOneLine()) {
            ...
            if (contents[0].equalsIgnoreCase(HttpBody.CONTENT_DISPOSITION)) {
                boolean checkSecondArg;
                if (currentStatus == MultiPartStatus.DISPOSITION) {
                    checkSecondArg = contents[1].equalsIgnoreCase(HttpBody.FORM_DATA);
                } else {
                    checkSecondArg = contents[1].equalsIgnoreCase(HttpBody.ATTACHMENT)
                            || contents[1].equalsIgnoreCase(HttpBody.FILE);
                }
                if (checkSecondArg) {
                    ...
                    for (int i = 2; i < contents.length; i++) {
                        ...
                        try {
                            ...
                            if (HttpBody.FILENAME.equals(name)) {
                ...
                Attribute attribute;
                try {
                    attribute = createAttribute(HttpHeaders.Names.CONTENT_TRANSFER_ENCODING, cleanString(contents[1]));
                } catch (NullPointerException e) {
                    throw new ErrorDataDecoderException(e);
                } catch (IllegalArgumentException e) {
                    throw new ErrorDataDecoderException(e);
                ...
                Attribute attribute;
                try {
                    attribute = createAttribute(HttpHeaders.Names.CONTENT_LENGTH, cleanString(contents[1]));
                } catch (NullPointerException e) {
                    throw new ErrorDataDecoderException(e);
                } catch (IllegalArgumentException e) {
                    throw new ErrorDataDecoderException(e);
                ...
                if (contents[1].equalsIgnoreCase(HttpBody.MULTIPART_MIXED)) {
                    if (currentStatus == MultiPartStatus.DISPOSITION) {
                    ...
                    for (int i = 1; i < contents.length; i++) {
                        if (contents[i].toLowerCase().startsWith(HttpHeaders.Values.CHARSET)) {
                            ...
                            Attribute attribute;
                            try {
                                attribute = createAttribute(HttpHeaders.Values.CHARSET, cleanString(values));
                            } catch (NullPointerException e) {
                                throw new ErrorDataDecoderException(e);
                            } catch (IllegalArgumentException e) {
                                throw new ErrorDataDecoderException(e);
                            ...
                            Attribute attribute;
                            try {
                                attribute = createAttribute(cleanString(contents[0]), contents[i]);
                            } catch (NullPointerException e) {
                                throw new ErrorDataDecoderException(e);
                            } catch (IllegalArgumentException e) {
                                throw new ErrorDataDecoderException(e);
        ...
        // Is it a FileUpload
        Attribute filenameAttribute = currentFieldAttributes.get(HttpBody.FILENAME);
        if (currentStatus == MultiPartStatus.DISPOSITION) {
    ...
    protected InterfaceHttpData getFileUpload(String delimiter) {
        ...
        if (encoding != null) {
            ...
            if (code.equals(HttpBody.TransferEncodingMechanism.BIT7.value())) {
                localCharset = HttpBody.US_ASCII;
            } else if (code.equals(HttpBody.TransferEncodingMechanism.BIT8.value())) {
                localCharset = HttpBody.ISO_8859_1;
                mechanism = TransferEncodingMechanism.BIT8;
            } else if (code.equals(HttpBody.TransferEncodingMechanism.BINARY.value())) {
        ...
        if (currentFileUpload == null) {
            ...
            try {
                ...
                if (contentTypeAttribute != null) {
                    ...
                    contentType = HttpBody.DEFAULT_BINARY_CONTENT_TYPE;
                }
                currentFileUpload = createFileUpload(cleanString(nameAttribute.getValue()),
    ...
    private FileUpload createFileUpload(String name, String filename, String contentType, String contentTransferEncoding, Charset charset, long size) {
    ...
    private void cleanMixedAttributes() {
        ...
        currentFieldAttributes.remove(HttpBody.FILENAME);
    }
    ...
    private static String[] splitMultipartHeader(String sb) {
        ...
        int valueStart;
        int valueEnd;
        nameStart = HttpBody.findNonWhitespace(sb, 0);
        ...
        valueStart = HttpBody.findNonWhitespace(sb, colonEnd);
        valueEnd = HttpBody.findEndOfString(sb);
    ...
    private static String[] splitHeaderContentType(String sb) {
        ...
        int cStart;
        int cEnd;
        aStart = HttpBody.findNonWhitespace(sb, 0);
        aEnd =  sb.indexOf(';');
        if (aEnd == -1) {
            return new String[] { sb, "", "" };
        }
        bStart = HttpBody.findNonWhitespace(sb, aEnd + 1);
        if (sb.charAt(aEnd - 1) == ' ') {
            aEnd--;
        }
        bEnd =  sb.indexOf(';', bStart);
        if (bEnd == -1) {
            bEnd = HttpBody.findEndOfString(sb);
            return new String[] { sb.substring(aStart, aEnd), sb.substring(bStart, bEnd), "" };
        }
        cStart = HttpBody.findNonWhitespace(sb, bEnd + 1);
        if (sb.charAt(bEnd - 1) == ' ') {
            bEnd--;
        }
        cEnd = HttpBody.findEndOfString(sb);
