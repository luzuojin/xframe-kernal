...
public class QueryString {
    ...
    protected static final int maxParams = 256;
    ...
    public QueryString(String uri, boolean hasPath) {
        ...
        this.queryStr = queryStr(uri, hasPath);
    }
    
    private String queryStr(String uri, boolean hasPath) {
        if (hasPath) {
            int pathEndPos = uri.indexOf('?');
            if (pathEndPos >= 0 && pathEndPos < uri.length() - 1) {
                return uri.substring(pathEndPos + 1);
            } else {
                return EMPTY_STRING;
            }
        } else {
    ...
    public QueryString(URI uri) {
        ...
        this.queryStr = uri.getRawQuery() == null ? "" : uri.getRawQuery();
    }
